...
if (!exists("CirceR", envir = .GlobalEnv)) {
  CirceR <- new.env(parent = .GlobalEnv)
}

CirceR$cohortExpressionFromJson <- function(expressionJson) {
...
CirceR$buildCohortQuery <- function(expression, options = NULL) {
...
CirceR$createGenerateOptions <- function(generateStats = FALSE, cohortId = NULL) {
  opts <- list(generateStats = generateStats)
  if (!is.null(cohortId)) opts$cohortId <- cohortId
  return(opts)
}

CirceR$cohortPrintFriendly <- function(expression) {
  if (is.character(expression)) {
    tryCatch({
...
CirceR$conceptSetListPrintFriendly <- function(conceptSets) {
...
tryCatch({
  if ("CirceR" %in% loadedNamespaces()) {
    for (fn_name in ls(CirceR)) {
      assignInNamespace(fn_name, get(fn_name, envir = CirceR), ns = "CirceR")
    }
    message("CirceRLite: patched existing CirceR namespace")
  }
}, error = function(e) {
  message("CirceRLite: loaded as standalone")
})

message("CirceRLite loaded successfully")
