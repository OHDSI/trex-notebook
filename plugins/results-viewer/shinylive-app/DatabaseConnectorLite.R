...
DatabaseConnector$createConnectionDetails <- function(dbms = "sqlite",
                                                       server = ":memory:",
                                                       ...) {
...
DatabaseConnector$connect <- function(connectionDetails = NULL, ...) {
  ...
  connection <- DBI::dbConnect(RSQLite::SQLite(), dbname = server)
  attr(connection, "dbms") <- "sqlite"
  return(connection)
}
...
DatabaseConnector$dbms <- function(connection) {
  ...
  if (is.null(dbms)) "sqlite" else dbms
}
