...
import JobsApp from './JobsApp.vue';

const CSS_LINK_ID = 'jobs-plugin-styles';
...
function injectPluginCss(uiFilesUrl: string): Promise<void> {
  ...
  // uiFilesUrl is the plugin's public directory (e.g. "/plugins/jobs-plugin/")
  // The CSS file lives directly inside that directory
  const base = uiFilesUrl
    ? uiFilesUrl.replace(/\/$/, '')
    : `${window.location.origin}/plugins/jobs-plugin`;
...
function injectMdiCss(): void {
  ...
  console.warn('[jobs] MDI font not present from host; icons may be missing.');
}
